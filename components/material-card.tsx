...
import { RemoteProductImage } from "@/components/remote-product-image";
...
export function MaterialCard({ category, index }: { category: MaterialCategory; index: number }) {
  ...
  return (
    <Link href={`/materiais/${category.slug}`} className="material-card">
      <span className="material-product-shot">
        <RemoteProductImage src={category.productImageUrl} fallbackSrc={category.image} alt={category.productImageAlt} />

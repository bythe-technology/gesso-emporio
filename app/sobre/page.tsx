...
import { BadgeCheck, MapPin, MessageCircle, PackageCheck, Users } from "lucide-react";
import Image from "next/image";
import { ADDRESS, MAPS_URL } from "@/lib/site-data";
import { buildWhatsAppUrl } from "@/lib/whatsapp";

export const metadata: Metadata = { title: "A loja", description: "Conheça a Gesso Empório, loja de materiais para construção a seco, acabamentos e iluminação em Registro/SP." };
    ...
    <>
      ...
      <section className="section story-section">
        <div className="container story-grid">
          <div className="story-image"><Image src="/images/loja-equipe-tratada.png" alt="Equipe e interior da Gesso Empório" fill priority sizes="(max-width: 900px) 100vw, 48vw" /></div>
      ...
      <section className="section"><div className="container location-card location-card-light"><div><span className="eyebrow eyebrow-dark"><span /> Nossa loja</span><h2>Esperamos você em Registro.</h2><p>{ADDRESS}. No galpão em frente à praça da Cecap.</p><div className="hours"><strong>Segunda a sexta</strong><span>7h30 às 18h</span><strong>Sábado</strong><span>7h30 às 13h</span></div><div className="inline-actions"><a className="button button-primary" href={MAPS_URL} target="_blank" rel="noreferrer"><MapPin size={18} /> Ver rota</a><a className="button button-outline" href={buildWhatsAppUrl()} target="_blank" rel="noreferrer"><MessageCircle size={18} /> WhatsApp</a></div></div><div className="map-pattern" aria-hidden="true"><span className="map-pin"><MapPin /></span></div></div></section>
    </>
  );
}
